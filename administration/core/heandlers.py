...
@dp.message(IsGroup())
async def group(message: types.Message) -> None:
...
async def orders_group(callback: types.CallbackQuery) -> None:
...
async def get_order_admin(callback: types.CallbackQuery) -> None:
...
async def ban(message: Message, state: FSMContext) -> None:
...
async def unban(message: Message, state: FSMContext) -> None:
    await bot.send_message(message.from_user.id, "Введите юзернейм пользователя без пробелов, с символом @ либо нажмите кнопку 'Отмена'.\nПример: @Pr1zrak0", reply_markup=cancel_markup)
...
async def admin_reg(message: types.Message) -> None:
...
async def adm_start(message: types.Message, state: FSMContext) -> None:
...
async def load_price_day(message: types.Message, state: FSMContext) -> None:
...
async def load_deposit(message: types.Message, state: FSMContext) -> None:
...
async def add_games(message: types.Message, state: FSMContext) -> None:
    data = await state.get_data()
...
async def reset_info(message: types.Message, state: FSMContext) -> None:
...
async def delete_game(message: types.Message, state: FSMContext) -> None:
    products_list = []
...
async def set_av_name(message: types.Message, state: FSMContext) -> None:
    products_list = []
    products = (await get_products_id_name())
    if products:
        for i in products:
...
async def del_admin_menu(message: Message, state: FSMContext) -> None:
...
async def set_av(message: types.Message, state: FSMContext) -> None:
    if not (await is_number(message)):
        await state.set_state(AddGame.photo)
        return
    data = await state.get_data()
...
async def delete_game_db(callback: types.CallbackQuery) -> None:
...
async def set_av(callback: types.CallbackQuery, state: FSMContext) -> None:
    product_name = (await get_product_name(int(callback.data)))
